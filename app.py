...
# 系统配置 - 修改并发数
MAX_GLOBAL_CONCURRENT = 50   # 全局最大并发数改为50
MAX_LOCAL_CONCURRENT = 5     # 单个网页最大并发数保持5
MAX_RETRIES = 5              
POLL_INTERVAL = 3            
MAX_POLL_COUNT = 300         
...
# 使用threading.Semaphore来控制并发数量
global_semaphore = threading.Semaphore(MAX_GLOBAL_CONCURRENT)
local_semaphore = threading.Semaphore(MAX_LOCAL_CONCURRENT)
...
def process_single_task(task, api_key, webapp_id, node_info):
    ...
    # 使用信号量来限制并发线程数量
    with global_semaphore:  # 全局并发控制
        with local_semaphore:  # 本地并发控制
            
            # 添加到处理中的任务集合
            with processing_lock:
                processing_tasks.add(task.task_id)
            ...
            try:
                ...
                # 从处理中的任务集合移除
                with processing_lock:
                    processing_tasks.discard(task.task_id)
...
def get_queue_stats():
    ...
    with processing_lock:
        global_processing = len(processing_tasks)
...
def start_new_tasks():
    ...
    # 检查是否有可用的并发槽位
    if stats['global_processing'] >= MAX_GLOBAL_CONCURRENT:
        return
        
    if stats['local_processing'] >= MAX_LOCAL_CONCURRENT:
        return
    ...
    with queue_lock:
        ...
        current_session_tasks = [t for t in task_queue if t.session_id == session_key]
        
        if not current_session_tasks:
            return
        
        # 移除要处理的任务
        task_to_process = current_session_tasks[0]
        task_queue.remove(task_to_process)
    
    # 启动任务处理线程
    thread = threading.Thread(
        target=process_single_task,
        args=(task_to_process, API_KEY, WEBAPP_ID, NODE_INFO)
    )
    thread.daemon = True
    thread.start()
...
def main():
    ...
    st.caption("高效处理 • 快速响应 • 实时更新 • 本地并发控制")
    
    st.info(f"⏱️ 预计处理时间: {DISPLAY_TIMEOUT_MINUTES}分钟 | 🔄 刷新间隔: {AUTO_REFRESH_INTERVAL}秒 | 🚀 全局并发: {MAX_GLOBAL_CONCURRENT} | 📱 本地并发: {MAX_LOCAL_CONCURRENT}")
    st.divider()
    ...
    with left_col:
        ...
        with st.expander("⚙️ 系统信息", expanded=False):
            st.text(f"并发控制: 本地Semaphore")
            st.text(f"会话: {get_session_key()}")
            st.text(f"配置: {MAX_GLOBAL_CONCURRENT}全局/{MAX_LOCAL_CONCURRENT}本地并发")
            
            # 显示信号量状态
            global_available = global_semaphore._value if hasattr(global_semaphore, '_value') else 'N/A'
            local_available = local_semaphore._value if hasattr(local_semaphore, '_value') else 'N/A'
            st.text(f"可用槽位: 全局{global_available} | 本地{local_available}")
        ...
        if not st.session_state.tasks:
            ...
            with col3:
                if st.button("📊 状态详情"):
                    ...
                    队列状态：
                    - 队列任务: {stats['queued']}
                    - 全局处理中: {stats['global_processing']}/{MAX_GLOBAL_CONCURRENT}
                    - 本地处理中: {stats['local_processing']}/{MAX_LOCAL_CONCURRENT}
                    - 已完成: {local_stats['success']}
                    - 失败: {local_stats['failed']}
                    """)
    ...
    <div style='text-align: center; color: #6c757d; padding: 15px;'>
        <b>🚀 RunningHub AI - 本地并发控制版</b><br>
        <small>去除Redis依赖 • 使用Semaphore控制并发 • 全局50/本地5并发限制</small>
    </div>
    """, unsafe_allow_html=True)
